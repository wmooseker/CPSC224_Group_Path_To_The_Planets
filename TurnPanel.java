...
public class TurnPanel extends JFrame implements ActionListener {
	...
	public TurnPanel(int numberOfTurns, Player player) throws IOException {
		thisPlayer = player;
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getSource() == seeScorecardButton) {
			ScorecardFrame scorecardFrame = new ScorecardFrame(thisPlayer.playerCard.upperCardScores,
					thisPlayer.playerCard.smallStraight, thisPlayer.playerCard.largeStraight,
					thisPlayer.playerCard.triForce, thisPlayer.playerCard.totalScore);
			scorecardFrame.setVisible(true);
		} else {
			// gets rid of the turn panel and returns to the takeTurn function
			// in the Game class
			this.setVisible(false);
		}
	}

	private void createMainPanel(int numberOfTurns) throws IOException {
		...
		Filler topBlankSpace = new Filler(new Dimension(0, 80), new Dimension(0, 80), new Dimension(Short.MAX_VALUE, 80));
		topBlankSpace.setOpaque(false);
		mainPanel.add(topBlankSpace);

		addRolledDicePanel();
		Filler bottomBlankSpace = new Filler(new Dimension(0, 300), new Dimension(0, 300), new Dimension(Short.MAX_VALUE, 80));
		bottomBlankSpace.setOpaque(false);
		mainPanel.add(bottomBlankSpace);
